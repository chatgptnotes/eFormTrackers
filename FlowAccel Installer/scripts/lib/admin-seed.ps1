...
function Seed-AdminUser {
    ...
    # 1. bcryptjs-hash via the just-installed backend dependency
    $node = (Get-Command node -ErrorAction Stop).Source
    Push-Location $BackendPath
    try {
        $hash = & $node -e "console.log(require('bcryptjs').hashSync(process.argv[1], 12))" $AdminPassword 2>&1
    } finally {
        Pop-Location
    }
    if ($LASTEXITCODE -ne 0 -or -not $hash -or -not ($hash.ToString()).StartsWith('$2')) {
        throw "bcryptjs hash failed: $hash"
    }
