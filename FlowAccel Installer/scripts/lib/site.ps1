...
function Write-DotEnv {
    ...
    # IIS serves the SPA and reverse-proxies the API on the same origin. The
    # backend still validates Socket.IO origins in production, so use the active
    # LAN address instead of the insecure wildcard value.
    $ip = Get-NetIPAddress -AddressFamily IPv4 -ErrorAction SilentlyContinue |
        Where-Object { $_.IPAddress -ne '127.0.0.1' -and $_.PrefixOrigin -ne 'WellKnown' } |
        Select-Object -First 1 -ExpandProperty IPAddress
    if (-not $ip) { $ip = '127.0.0.1' }
    $allowedOrigin = "http://$ip"
    $nodeEnv = 'production'
    $pollerEnabled = if ($Config.JotformApiKey) { '1' } else { '0' }
