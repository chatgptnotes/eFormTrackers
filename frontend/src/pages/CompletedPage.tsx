          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className="p-4 text-center">
                ...
                {/* iframe: loads the URL in a separate browsing context so
                    JotForm session cookies (SameSite=None+Secure) are sent
                    the same way as the user's "Open in new tab" workflow.
                    Renders the PNG inline; if not logged in to JotForm,
                    the iframe shows JotForm's login page. */}
                <iframe
                  src={viewSignature.url}
                  title="Signature"
                  className="w-full h-[380px] border border-gray-200 rounded-lg bg-white"
