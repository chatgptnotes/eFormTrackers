...
/**
 * Modal that asks the user which native JotForm action (approve/reject) to sync
 * when the dashboard detects an out-of-band status change.
 */
export function SyncConfirmModal({ submission, loading, onConfirm, onClose }: Props) {
  return (
    <AnimatePresence>
      {submission && (
        <motion.div
