...
export default function NotificationBell() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await apiFetch<Notification[]>(`/api/notifications?user_email=${encodeURIComponent(user.email)}`);
        if (data && data.length > 0) setNotifications(data);
      } catch {}
    };
    load();

    // Poll every 30s for new notifications (replaces Supabase realtime)
  ...
  const markAllRead = async () => {
    ...
    if (user) await apiFetch(`/api/notifications/read-all?user_email=${encodeURIComponent(user.email)}`, { method: 'PUT' });
  };
