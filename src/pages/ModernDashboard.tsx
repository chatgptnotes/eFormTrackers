...
export default function ModernDashboard({ data }: Props) {
  ...
  const fetchAndShowSignature = useCallback(async (submissionId: string, level: number, taskId: string) => {
    ...
    try {
      const { data } = await supabase.from('jf_signatures').select('signature_url, approver_name').eq('submission_id', submissionId).eq('level', level).order('created_at', { ascending: false }).limit(1).single();
