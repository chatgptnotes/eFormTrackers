...
REM  Wraps the pre-built FlowAccel-Setup-1.0.1.exe in a 7-Zip
...
REM      and produced output\FlowAccel-Setup-1.0.1.exe
REM ============================================================

set "REPO=%~dp0"
set "SRC=%REPO%FlowAccel Installer\output\FlowAccel-Setup-1.0.1.exe"
...
echo [INFO] Splitting FlowAccel-Setup-1.0.1.exe into 90 MB 7-Zip volumes...
REM -mx=0 = store only (no recompression; .exe payload is already LZMA-packed)
REM -v90m = 90 MB per volume
"%SEVENZ%" a -t7z -mx=0 -v90m "%OUT%\FlowAccel-Setup-1.0.1.7z" "%SRC%"
if errorlevel 1 (
...
powershell -NoProfile -Command ^
  "$out = '%OUT%';" ^
  "$src = '%SRC%';" ^
  "$parts = Get-ChildItem -Path $out -Filter 'FlowAccel-Setup-1.0.1.7z.*' | Sort-Object Name;" ^
  "$lines = @();" ^
  ...
  "$lines += ($exeHash + '  extracted (FlowAccel-Setup-1.0.1.exe)');" ^
  "Set-Content -Path (Join-Path $out 'SHA256SUMS.txt') -Value $lines -Encoding ASCII"
if errorlevel 1 exit /b 1

REM Hard-fail if any part > 95 MB
powershell -NoProfile -Command ^
  "$bad = Get-ChildItem -Path '%OUT%' -Filter 'FlowAccel-Setup-1.0.1.7z.*' | Where-Object { $_.Length -gt 95MB };" ^
